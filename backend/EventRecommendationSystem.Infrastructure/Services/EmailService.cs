...
Команда Event Recommendation System
    ...
    public async Task<bool> SendRegistrationVerificationAsync(string toEmail, string code, string username)
    {
        try
        {
            Console.WriteLine($"[EMAIL] Отправка кода подтверждения регистрации на {toEmail}...");

            var message = new MimeMessage();
            message.From.Add(new MailboxAddress(_emailSettings.SenderName, _emailSettings.SenderEmail));
            message.To.Add(new MailboxAddress(username, toEmail));
            message.Subject = "Подтверждение регистрации - Event Recommendation System";
            ...
            var bodyBuilder = new BodyBuilder
            {
                HtmlBody = $@"
                    ...
                    <body>
                        <div class='container'>
                            ...
                            <div class='content'>
                                <p>Здравствуйте, <strong>{username}</strong>!</p>
                                <p>Вы создаёте аккаунт в Event Recommendation System. Чтобы завершить регистрацию, введите код подтверждения:</p>
                                <div class='code-box'>
                                ...
                                <div class='warning'>
                                    <strong>⚠️ Внимание!</strong> Если вы не регистрировались в нашей системе, просто проигнорируйте это письмо.
                                </div>
                                <p>С уважением,<br>Команда Event Recommendation System</p>
                            </div>
                            <div class='footer'>
                ...
                ",
                TextBody = $"Код подтверждения регистрации: {code}\nКод действителен 15 минут."
            };
            ...
            Console.WriteLine($"[EMAIL] Код подтверждения регистрации успешно отправлен на {toEmail}");
            return true;
        }
        catch (Exception ex)
        {
            Console.WriteLine($"[EMAIL ERROR] Ошибка отправки кода подтверждения регистрации: {ex.Message}");
            return false;
        }
    }
